    ...
    case 
      when length(products.country) = 3 then 
        case upper(products.country)
          ...
          when 'ZWE' then 'ZW' when 'CYM' then 'KY' when 'AND' then 'AD' when 'GUM' then 'GU' when 'MTQ' then 'MQ' 
          when 'PYF' then 'PF' when 'CUW' then 'CW' when 'PRI' then 'PR' when 'JEY' then 'JE' else substr(products.country, 1, 2)
        end
